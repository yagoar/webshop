...
\begin{figure}[th!]
	\centering
	\includegraphics[width=0.8\linewidth]{bilder/kap7/JpaRepo}
	\caption{CategoryDao.java}
	\label{fig:CategoryDao}
\end{figure}

\cref{fig:CategoryDao} zeigt eine solche \acs{DAO}-Klasse.
...
Über Parameter dieser Annotationen können neben der Ziel-Entität für die Relationen weitere Einstellungen gemacht werden, auf die hier nicht näher eingegangen werden soll.
...
Hibernate bietet dafür drei grundlegende Varianten an:
\paragraph{Single table}$\;$ \\

\paragraph{Table per class}$\;$ \\
\paragraph{Joined}$\;$ \\
